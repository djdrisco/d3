...
d3.layout.packnetwork = function() {
  var hierarchy = d3.layout.hierarchy().sort(d3_layout_packSort),
      padding = 0,
      size = [1, 1],
      radius;

  function packnetwork(d, i) {
  ...
  packnetwork.size = function(_) {
    if (!arguments.length) return size;
    size = _;
    return packnetwork;
  };

  packnetwork.radius = function(_) {
    if (!arguments.length) return radius;
    radius = _ == null || typeof _ === "function" ? _ : +_;
    return packnetwork;
  };

  packnetwork.padding = function(_) {
    if (!arguments.length) return padding;
    padding = +_;
    return packnetwork;
  };

  return d3_layout_hierarchyRebind(packnetwork, hierarchy);
};
